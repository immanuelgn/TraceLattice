...
export default function EthicsPage() {
  const guardrails = [
    ...
    [Ban, "No offensive behavior", "No broad crawling, exploitation, brute force, bypass, payload injection, or vulnerability attacks."],
  ...
  return <div className="page-shell container"><SectionHeading eyebrow="Defensive boundaries" title="Useful intelligence without crossing the line." copy="TraceLattice is constrained to bounded passive analysis of public web origins." /><DisclaimerBanner /><div className="guardrail-grid">{guardrails.map(([Icon, title, copy]) => <article className="glass" key={title}><Icon /><h2>{title}</h2><p>{copy}</p></article>)}</div><section className="glass content-panel"><h2>Acceptable use</h2><p>Use TraceLattice on public websites for educational review, defensive engineering, and vendor evaluation. Do not use it to harass operators, evade controls, target private systems, or represent heuristic output as a professional audit or legal conclusion.</p></section></div>;
}
