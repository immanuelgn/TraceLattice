...
export function RecentScans() {
  ...
  if (!items.length) {
    return (
      <EmptyState
        title="No local scan history yet"
        copy="Run a live or demo scan, then choose Save locally. Only compact summaries are stored in this browser."
      />
    );
  }
          ...
          <article className="glass recent-card" key={`${item.domain}-${item.scannedAt}`}>
            <div className="recent-head">
              <div>
                ...
                <div className="recent-date"><Clock3 size={13} />{new Date(item.scannedAt).toLocaleString()}</div>
              </div>
              <div className="mini-grade">{item.score}<small>score</small></div>
            </div>
            <Pill tone={item.score >= 80 ? "green" : item.score >= 60 ? "amber" : "red"}>{item.riskLabel}</Pill>
            <div className="recent-stats">
              <span><strong>{item.trackerCount}</strong> trackers</span>
              <span><strong>{item.cookieIssueCount}</strong> cookie issues</span>
              <span><strong>{item.thirdPartyCount}</strong> third parties</span>
            </div>
